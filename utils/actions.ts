...
import { createReviewSchema, ImageSchema, ProfileSchema, propertySchema, validateFieldsWithZodSchema } from "./schemas";
import db from './db'
...
export const createReviewAction = async (prevState: any, formdata: FormData) => {
    ...
    try {
        const rawData = Object.fromEntries(formdata);
        const validatedFileds = validateFieldsWithZodSchema(createReviewSchema, rawData);
